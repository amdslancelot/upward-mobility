...
- The token bill of a run is calls × context: everything added to the conversation is re-read by every later call, and every avoidable call pays a full re-read of everything so far. So batch independent tool calls into one message instead of spending a call per probe; pipe a verbose gate's output to a file, judge pass/fail by the exit code, and read back only the tail as the logged evidence while it passes, opening the full log only on failure (a failure must be seen in full); fetch research through a read-only subagent that returns a short digest instead of carrying raw material to the end of the run; and skip conversational task-tracker tools in headless runs — plan.md is the task list. (One measured run spent a third of its total tokens re-reading 45K of early web research it needed for ten minutes.)
