...
The one who did the work doesn't verify their own output — and authoring the plan and its acceptance criteria counts as doing the work. The commander who wrote the criteria may dispatch reviews and triage findings, but must not perform the verification personally: the criteria author's blind spots are exactly what the review exists to catch, and "the main thread ran the gates itself" is both self-verification of the plan and the single largest token leak measured in practice. Verification always goes to a freshly spawned agent (general-purpose, never continued via `SendMessage` from the agent that did the work), because whoever did the work carries the author's point of view and will mentally fill in gaps the artifact never actually states.
...
- **The open-ended hunt pass (required for any multi-item task, never skippable)**: at least one review dispatch must be adversarial and unscripted — attach the original user request verbatim, declare the acceptance criteria suspect, and ask "what breaks in real use that the criteria don't cover?" A reviewer that only confirms the checklist inherits the checklist's blind spots: it verifies the work against the plan when what's needed is to verify the plan against reality. The commander dispatches this pass once all task items are checked off, to a fresh general-purpose agent, using the "Reality check (consumer-seat smoke)" review type in `upward-ops-dispatch`. The plan's consumer-seat reality check (per `upward-ops-plan`) does not discharge this pass — it is executed *by* this pass: the same fresh-context dispatch boots and exercises the artifact from the consumer's seat *and* re-attacks the criteria against the original request. One dispatch, both halves, and no part of it is handed back to the plan's author. This reviewer is also the only actor that re-executes gates the builders already ran — the builders' reported evidence plus this one independent run is the complete verification budget; a commander adding a third pass over the same gates adds tokens, not assurance.
