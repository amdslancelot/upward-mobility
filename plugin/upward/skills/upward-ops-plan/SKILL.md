...
Four defect classes have shipped repeatedly across measured builds of unrelated tasks — each one passed every build gate and died in production state no gate reaches. When the plan touches the matching area, copy the line into that item's acceptance criteria so the gate and the reviewer both check it:
...
4. **After the last main-thread item (N−1), load `upward-ops-review` via the Skill tool — and load `upward-ops-dispatch` too if this run hasn't loaded it yet** — then dispatch exactly one fresh-context consumer-seat review (foreground) per the review-dispatch section of `upward-ops-dispatch`. This single dispatch does two jobs at once: it executes the consumer-seat reality check (task N of the plan) — booting and exercising the artifact per rule 4's ladder, including running any repeatable consumer path twice — and it attacks the acceptance criteria against the original request, which travels verbatim in its prompt. It does not re-run install/build/test gates you already ran and logged with execution evidence; its scope is the real entrypoints and the blind spots, not your checklist. You never perform this review yourself — authoring the criteria counts as doing the work, and the author verifying against their own criteria is the Goodhart loop this workflow exists to break. When the review comes back, check item N off on the reviewer's reported evidence; a NOT DONE consumer-seat report is itself a finding — take it into step 5.
