...
- [ ] 1. <task name>
  ...
  - Acceptance / Review: <decidable done-criterion; whoever executes the item — normally the main thread itself — runs the gate once when the item finishes and logs the evidence; no per-item verifier agent; the end-of-run consumer-seat reviewer is the only independent pass>
- [ ] 2. ...
...
2. **Dispatch only when it pays**: workstreams that are genuinely parallel and independent, or raw material that would flood this context. Before the first such dispatch, load `upward-ops-dispatch` via the Skill tool and follow its brief template. Prefer foreground dispatch — every background dispatch wakes this thread again with a paid notification round.
...
| The main context executes items itself; one scoped reviewer is the only independent pass | Token burn from per-item subagent cold-starts and from gates being re-run by extra actors — and quality loss from a cheaper builder model writing the shipped code |
