...
1. **Fresh-context checks** — the end-of-run consumer-seat review, a read-back, a second opinion. Independence is the product, and it cannot be produced inside the context that did the work. These agents may execute the artifact and its gates when the declared review-effort level is MED or HIGH (booting it, dry-running a deploy, feeding it real input — see "Review-effort levels" below); at LOW, the default, they read and report only; what they never do at any level is edit it.
...
| Reality check (consumer-seat smoke) | Does the artifact work when used the way its real consumer uses it — at MED or HIGH effort, boot it, deploy it, feed it a real input, run any repeatable path twice; at LOW (the default), a static adversarial sweep plus the criteria attack, with no execution | general-purpose agent; the depth comes from the declared effort level below — a real run happens only at MED or HIGH, "it builds" never qualifies as a run, and at no level does it re-run install/build/test gates the worker already logged with execution evidence |

### Review-effort levels (every review dispatch declares one)

Verification depth is a budget decision, not a draw: two measured runs on identical rules drew internal review costs differing by nearly 2×, and nothing in the rules knew the difference. So every review dispatch names one of three effort levels in its delegation brief, and the reviewer's report must state the level it executed and show that level's evidence floor. The budgets are scope-relative — an absolute token number does not transfer between projects — so each level is defined by what it must do, and its cost scales with the artifact under review.

| level | mandatory evidence floor | scope-relative spend |
|---|---|---|
| **LOW** (default) | Read the full source; statically hunt for defects that pass build gates but fail in real use — state that survives across runs and restarts, repeated or concurrent invocations, every boundary the artifact's inputs actually cross (a function call, a request, a config load), inputs larger or stranger than the fixtures — naming each area checked, with file:line evidence; every finding carries a concrete failure scenario. No execution claims of any kind — the verdict carries a **STATIC-ONLY** marker so the main thread and the user know what was not checked, and when the artifact is executable the report also names the level that would check it (a recommended-level line). | On the order of one full read of the artifact plus reporting. |
| **MED** | LOW's floor, plus: install, run any self-check the artifact ships, production build, boot with dummy env; probe one feature path, and — when the artifact has them — its auth/entry gate and one secret- or token-guarded path with a wrong and a right credential. "It builds" does not qualify as a run. | LOW plus one build-boot-probe cycle — measured at roughly 1.1–1.3× LOW. |
| **HIGH** | MED's floor, plus: demonstrate the top finding end-to-end against the running artifact, and exercise at least one unattended or repeatable path twice (a scheduled job before any interactive use, a re-import of the same input, a concurrent trigger). A HIGH-severity finding reported without a demonstration needs an explicit *undemonstrable-because* line. | Roughly 2–2.5× LOW. |

Selection rule: the level is the user's decision, recorded in the run's brief, and absent any instruction it is **LOW**. The brief may carry MED or HIGH only when the user actually asked for that depth — the planning model never writes a higher level in on its own initiative (in an unattended run especially, a self-selected level is indistinguishable from silent escalation). The way to argue for more depth is the report's recommended-level line; the spend decision stays with the user. LOW's blind spot is real and accepted as the default trade: defect classes that only exist at runtime (a broken build-arg chain, a boot failure, framework CSRF behavior, memory exhaustion under a real input) are invisible to reading, and the STATIC-ONLY marker exists precisely so that blind spot is declared rather than hidden. LOW's *measured* failure mode is one step subtler: fixes applied to LOW's own findings ship unverified — the review runs before the fix exists, so nothing downstream ever executes it, and a measured build's only HIGH was inside exactly such a fix. That gap is closed on the main thread, not by escalating the review: a post-review fix that adds new behavior gets its own execution gate per `upward-ops-plan` step 5, at any review level.
...
## 3. Review (reviewer/fresh-context, sonnet) — for type/model selection and prompt add-ons, see "Dispatching a review task" above. The end-of-run consumer-seat pass counts as ONE focus: its verification half (static at LOW, boot-and-exercise at MED or HIGH) and its criteria-attack half travel in the same single dispatch, never two.
...
Effort level: [LOW (default) | MED | HIGH] — copy the declared level's evidence floor from "Review-effort levels" above into the acceptance criteria; the report must state the level it executed, and a LOW verdict carries the STATIC-ONLY marker.
