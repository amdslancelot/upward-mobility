...
|---|---|---|
| **LOW** (default) | Read the full source; check every Known-killers class (the checklist lives in `upward-ops-plan`) statically, with file:line evidence per class; every finding carries a concrete failure scenario. No execution claims of any kind — the verdict carries a **STATIC-ONLY** marker so the main thread and the user know what was not checked, and when the artifact is executable the report also names the level that would check it (a recommended-level line). | On the order of one full read of the artifact plus reporting. |
| **MED** | LOW's floor, plus: install, run any self-check the artifact ships, production build, boot with dummy env; probe the auth/entry gate, one secret- or token-guarded path with a wrong and a right credential, and one feature path. "It builds" does not qualify as a run. | LOW plus one build-boot-probe cycle — measured at roughly 1.1–1.3× LOW. |
