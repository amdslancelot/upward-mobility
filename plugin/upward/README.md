...
- **Verification isn't self-verification.** Each item's completion claim stands on the worker's own execution evidence (build, test, real run); the final done-and-good-enough verdict gets exactly one independent pass — a fresh-context consumer-seat review, never performed by whoever did the work. **That review runs at LOW effort by default: a static-only pass** (full source read plus an adversarial sweep for defects that only bite in real use; no build, no boot) whose verdict carries a STATIC-ONLY marker naming what was not checked. If you want execution-backed verification — build, boot, and probes (MED) or a live demonstration of findings (HIGH) — request the level in your prompt; the plugin recommends deeper levels when they matter but never escalates the spend on its own.
...
| **MED** | Installs and boots the artifact, runs any self-check it ships, probes one feature path (and the auth/entry gate, when the artifact has one). | You want the review to actually run the thing, not just read it. |
