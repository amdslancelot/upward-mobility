# How to dispatch a review task: type, model, prompt structure, disposing of findings

> Reader: the main-conversation model. Answers "when I need to review a batch of output, which agent type, which model, what prompt, and what do I do once findings come back."
> Complements `ops/model-dispatch.md`: that file covers whether to dispatch at all; this file covers how to dispatch a review specifically.
> Consolidated from the 2026-07-06 Fable 5 session transcript.
> For the full plan → execute → review → revise task-loop skeleton, see the root-level `WEAK-MODEL-PROMPT-GUIDE.md`.
...
|---|---|---|
| Diff/PR correctness | Does this change have bugs | The `/code-review` skill, or caveman:cavecrew-reviewer (consumes a git diff) |
| Over-engineering | Abstractions/dependencies that shouldn't exist | The `/ponytail-review` skill (hunts complexity only; if not installed, use general-purpose with that focus) |
| Document consistency | Cross-file contradictions, placeholders, broken cross-references, ambiguous sentences a weaker model would misread | general-purpose agent + `ops/prompt-templates.md` #5 template |
...
The rubric in one line: **use a diff tool to review a diff, use general-purpose with a custom checklist to review content.** A specialized tool's field of view is hard-coded — anything outside that view, it stays silent about. Silence isn't the same as "no problem."

## 2. fresh-context isn't a formality, it's a test condition

Reviews always get a newly-opened agent (never SendMessage to continue a prior agent), for two reasons:
...
## 3. Model choice: haiku verifies form, sonnet verifies meaning
...
## 4. The required structure of a review prompt (start from `ops/prompt-templates.md` #5, then add this)
...
## 5. Obligations once findings come back (same playbook as phase 4 of `WEAK-MODEL-PROMPT-GUIDE.md`)
...
3. After fixing, do a lightweight verification pass (grep/wc-level checks are fine to do yourself — mechanical confirmation doesn't count as bulk reading, no need to dispatch another round for it).
